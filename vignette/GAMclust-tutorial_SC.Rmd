...
For single cell data, take 10,000 genes for GAM-clustering analysis. 
To do this while [preprocessing data with Seurat pipeline](https://satijalab.org/seurat/articles/sctransform_vignette), set `variable.features.n = 10000` in `SCTransform()` function.
In case of [preprocessing multi-sample data](https://satijalab.org/seurat/articles/integration_introduction), set `nfeatures=10000` in `SelectIntegrationFeatures()`).
...
<img src="/nfs/home/againullina/GAMclust/vignette/results/m.1.svg" type="image/svg+xml" />
...
```{r}
head(read.tsv(paste0(work.dir, "/m.1.genes.tsv")))
```
...
for(i in 1:length(m.gene.list)){
  ...
  fgsea::plotCoregulationProfileReduction(m.gene.list[[i]], 
                                          seurat_object, 
                                          reduction = "umap")
